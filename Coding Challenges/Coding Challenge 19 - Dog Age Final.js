...
dogs.forEach((dog) => {
    if (dog.owners.includes('Sarah'))
        console.log(dog.recFood > dog.curFood ? 'Eats to little' : 'Eats to much');
});
// Alternative
const dogSarah = dogs.find((dog) => dog.owners.includes('Sarah'));
console.log(
    `Sarah's dog is eating too ${dogSarah.curFood > dogSarah.recFood ? 'much' : 'little'} food`
);
...
// 7.
const okDogs = dogs.filter(checkEatingOkay);
console.log(okDogs);
